Repository.Local = { Methods:  
{
    initialize: function(callbackFn) {
        ...
        if (typeof __qregPVSettings === 'object') {
            me.qregPVSettings = Ext.apply(me._qregPVSettings(), __qregPVSettings);
        ...
        if (!me._initOnce) {
            ...
            me.initializeLatestDate(function(resp) {
                if (resp && resp.success) {
                    collectionDate && collectionDate.setHtml(me.getCurrentDate('F Y'));
                    me.getLocal()._unitStore.load({
                        callback: function() {
                            me.getLocal()._indicatorStore.load({
                                callback: function() {
                                    me._initializeWithDialog(function() {
    ...
    getCurrentDate: function(toStringFormat) {
        var d = new Date(this.getCurrentYear() + '/' + this.getCurrentMonth() + '/01');
        if (toStringFormat) {
    ...
    initializeUnitStore: function() {
        ...
        this.getLocal()._unitStore = Ext.StoreManager.lookup('QregPVUnitStore');

        if (!this.getLocal()._unitStore) {
            this.getLocal()._unitStore = Ext.create('Ext.data.Store', Ext.apply({
                storeId: primaryId,
                proxy: {
                    type: 'ajax',
                    cors: true, //TODO: remove in production
                    url: '/api/metadata/domains/map/5656',
                    reader: {
                        type: 'objecttoarray',
                ...
                listeners: {
                    load: function(s, records) {
                        var secondaryStore = Ext.StoreManager.lookup(secondaryId);
                        if (secondaryStore) {
            ...
            this.getLocal()._unitStoreSecondary = Ext.create('Ext.data.Store', Ext.apply({
                storeId: secondaryId
            }, conf));
        }
    },
        ...
        Ext.Ajax.request({
            ...
            success: function(response) {
                ...
                if (currYear) {
                    ...
                    collectionDate && collectionDate.setHtml(currYear + '-' + currMonth + '-01');
    ...
    initializeIndicatorStore: function() {
        this.getLocal()._indicatorStore = Ext.StoreManager.lookup('QregPVIndicatorStore') ||
            Ext.create('Ext.data.Store', {
                ...
                proxy: {
                    type: 'ajax',
                    url: '/api/metadata/domains/map/5655',
                    reader: {
                        type: 'objecttoarray',
    ...
    initializeCountStore: function() {
        ...
        me.getLocal()._countStoreKrs = Ext.StoreManager.lookup('QRegPVCountStoreKrs') ||
            Ext.create('Ext.data.Store', {
                storeId: 'QRegPVCountStoreKrs',
                fields: ['Q_Varde', 'Date', 'Q_Unit', 'Q_Year', 'Q_Month', 'Q_Indicator', 'Sickness'],
                filters: [
                    function(v) {
        ...
        me.getLocal()._countStoreHyp = Ext.StoreManager.lookup('QRegPVCountStoreHyp') ||
            Ext.create('Ext.data.Store', {
                storeId: 'QRegPVCountStoreHyp',
                fields: ['Q_Varde', {
                    name: 'Date',
                    convert: function(v, r) {
                        return Ext.Date.format(new Date(r.get('Q_Year') + '/' + r.get('Q_Month') + '/1'), 'F Y');
                    }
                }, {
                    name: 'Q_Unit',
                    convert: function(v) {
                ...
                }, 'Q_Month', 'Q_Year', 'Q_Indicator', {
                    name: 'Sickness',
                    convert: function(v, r) {
                        return Ext.String.startsWith(r.get('Q_Indicator') + '', '10') ? 'hypertoni' : 'kranskärlssjukdom';
                    }
                }],
                proxy: {
                    type: 'ajax',
                ...
                loadCountData: function(HSAID, year, month) {
                    var loadFn = function() {
                        ...
                        this.proxy.url = Ext.String.format(url, HSAID, year, month, 1022, 2023);
                        this.load();
                    };
                    if (this.isLoading()) {
        ...
        if (currentUnit) {
            me.loadCountData(currentUnit, me.getCurrentYear(), me.getCurrentMonth());
        }
    },
    getMainStore: function(conf) {
        var me = this,
            ...
            first = false;
        if (!mainStore) {
            startDate = Ext.Date.format(Ext.Date.add(me.getCurrentDate(), Ext.Date.YEAR, -1), 'Y-m-d');
            first = true;
            mainStore = Ext.create('Ext.data.Store', {
                ...
                fields: [{
                    ...
                    name: 'Date',
                    convert: function(v, r) {
                        return Ext.Date.parse(r.get('Q_Year') + '-' + r.get('Q_Month'), 'Y-n');
                    }
                }, {
                    name: 'IndicatorName',
                    convert: function(v, record) {
                        return me.getIndicatorName(record.get('Q_Indicator'));
                    }
                }, {
                    name: 'Q_Month',
                    type: 'number'
                ...
                proxy: {
                    ...
                    reader: {
                        type: 'twounitreader',
                        rootProperty: 'data',
                        unitSpecificFields: ['Q_Varde', 'Q_Unit', 'Q_Namnare']
                    }
                },
                loadNewUnitData: function(firstHSAID, secondHSAID, skipLoad) {
                    var loadFn = function() {
                        var url = '//stratum.registercentrum.se/api/registrations/form/2179?query=Q_Date%20gt%20' + startDate + ',Q_Unit%20in%20{0}|{1}';
                        var filters;
                        Ext.isFunction(this.beforeLoadFn) && this.beforeLoadFn.call(this);

                        this.proxy.url = Ext.String.format(url, firstHSAID, secondHSAID);
                        ...
                        if (!skipLoad) {
                            ...
                            this.load({
                                scope: this,
                                callback: function() {
                                    Ext.isFunction(this._currFilter) && this.filter(this._currFilter);
                                }
                            });
                        }
                    };
                ...
                listeners: {
                    load: function() {
                        Ext.isFunction(this.onLoadFn) && this.onLoadFn.apply(this, arguments);
                    }
                }
            });
            mainStore.loadNewUnitData(me.getPrimaryUnit(), me.getSecondaryUnit());
        }
        ...
        if (Ext.isObject(conf)) {
            ...
            if (!first && conf.triggerLoadFn) {
                mainStore.fireEvent('load', mainStore, mainStore.getRange());
            }
    ...
    loadCountData: function(HSAID, year, month) {
        if (this.getLocal()._countStoreHyp) {
            this.getLocal()._countStoreHyp.loadCountData(HSAID, year, month);
        }
    },
    createPrimaryUnitInitilizer: function(callbackFn) {
        ...
        Ext.create('Ext.window.Window', {
            ...
            bodyPadding: 10,
            glyph: 0xf013,
            items: [Ext.create('QRegPV.ClinicCombo', {
                isPrimary: true,
                width: '100%',
    ...
    _getAlphaColor: function(color, alpha) {
        ...
        return Ext.String.format('rgba({0}, {1}, {2}, {3})', aColor.r, aColor.g, aColor.b, alpha);
    },
    getPrimaryColor: function(alpha) {
        var color = this.qregPVSettings.colors && this.qregPVSettings.colors[1] || '#555';
        return Ext.isNumeric(alpha) ? this._getAlphaColor(color, alpha) : color;
    },
    getSecondaryColor: function(alpha) {
        var color = this.qregPVSettings.colors && this.qregPVSettings.colors[0] || '#999';
        return Ext.isNumeric(alpha) ? this._getAlphaColor(color, alpha) : color;
    },
    ...
    _getUnit: function(isSecondary) {
        var unit;
        try {
            unit = isSecondary ? this.getLocal()._secondaryClinic : this.getLocal()._primaryClinic;
        } catch (e) {
            unit = null;
        } finally {
            unit = !unit ? (isSecondary ? this.getRegionHSAID() : this.getHSAID()) : unit;
        }
        return unit;
    },
    ...
    setRankingSprites: function(chart, indicator, place, total) {
        chart.setSprites([{
            ...
            type: 'text',
            text: Ext.String.format('{0} ({1} av {2} från vänster)', indicator, place, total),
            textAlign: 'left',
            fontSize: 14,
            fontFamily: 'open_sans, sans-serif',
            width: 200,
    ...
    setRankingTitle: function(store, chart, subtitleFn) {
        var count = store.count(),
            ...
            indicator = Ext.isFunction(subtitleFn) ? subtitleFn(store.first()) : this.getIndicatorName(store.first().get('Q_Indicator'));
    ...
    _initializeDefinitions: function() {
        ...
        !Ext.ClassManager.isCreated('QRegPV.TipStore') && Ext.define('QRegPV.TipStore', {
         ...
         */
        !Ext.ClassManager.isCreated('QRegPV.TipChart') && Ext.define('QRegPV.TipChart', {
            ...
            constructor: function(config) {
                ...
                config.axes = [{
                    ...
                    renderer: function(v) {
                        return Ext.isDate(v) ? Ext.Date.format(v, v.getMonth() === 0 ? 'Y' : 'M') : v;
                    },
                    style: {
                        strokeStyle: '#ccc'
                    }
        ...
        !Ext.ClassManager.isCreated('QRegPV.ConfigContainer') && Ext.define('QRegPV.ConfigContainer', {
        ...
        !Ext.ClassManager.isCreated('QRegPV.ConfigPanel') && Ext.define('QRegPV.ConfigPanel', {
            extend: 'Ext.panel.Panel',
            collapseFn: function(p) {
                p.normalWidth = p.normalWidth || p.ownerCt && p.ownerCt.getWidth();
                p.setWidth(p.collapsed ? p.normalWidth : p.collapsedWidth);
        ...
        !Ext.ClassManager.isCreated('QRegPV.ChartSaveButton') && Ext.define('QRegPV.ChartSaveButton', {
        ...
        !Ext.ClassManager.isCreated('QRegPV.ClinicCombo') && Ext.define('QRegPV.ClinicCombo', {
            ...
            initComponent: function() {
                ...
                Ext.apply(me, {
                    _singleListeners: {},
                    store: 'QregPVUnitStore' + (me.isPrimary ? '' : 'Secondary'),
                    fieldStyle: {
                        ...
                        'border-color': '#ccc',
                        'border-style': 'solid',
                        'border-left-color': me.isPrimary ? repo.getPrimaryColor() : repo.getSecondaryColor(),
                        // 'border-radius': '5px',
                        // 'margin-top': '4px',
                        'padding': '5px'
                    },
                    value: me.isPrimary ? repo.getPrimaryUnit() : repo.getSecondaryUnit(),
                    fieldLabel: me.fieldLabel || (me.isPrimary ? 'Välj primär vårdcentral' : 'Välj vårdcentral för jämförelse'),
                    columnDataIndex: me.isPrimary ? 'Q_Varde_0' : 'Q_Varde_1'
                });
            ...
            listeners: {
                select: function(cb, records) {
                    ...
                    unitId = Ext.isArray(records) ? records[0].get('UnitID') : records.get('UnitID');
                    if (this.isPrimary) {
                        repo.getLocal()._primaryClinic = unitId;
                        if (!this.skipStoreLoad) {
                            repo.loadCountData(unitId, repo.getCurrentYear(), repo.getCurrentMonth());
                        }
                    ...
                    if (!this.skipStoreLoad) {
                        repo.getMainStore().loadNewUnitData(repo.getPrimaryUnit(), repo.getSecondaryUnit());
                    }
                    ...
                    window.setTimeout(function() {
                        ...
                        if (parentWindow) {
                            ...
                            parentPanel.collapse(Ext.Component.DIRECTION_TOP, Ext.isIE);
                        }
                    }, 100);
                },
                focus: function() {
            ...
            addSingleListener: function(event, eventFn) {
                ...
                this.addListener(event, this._singleListeners[event], this);
            },
        ...
        !Ext.ClassManager.isCreated('QRegPV.TwoUnitReader') && Ext.define('QRegPV.TwoUnitReader', {
            ...
            setFirstUnit: function(hsaid) {
                this.firstUnit = typeof hsaid === 'string' ? hsaid : this.firstUnit;
            },
            setSecondUnit: function(hsaid) {
                this.secondUnit = typeof hsaid === 'string' ? hsaid : this.secondUnit;
            },
            ...
            getFirstFieldInstance: function(field) {
                return Ext.String.format(this.fieldFormat, field, 0);
            },
            getSecondFieldInstance: function(field) {
                return Ext.String.format(this.fieldFormat, field, 1);
            },
            getResponseData: function(aResponse) {
                ...
                try {
                    ...
                    Ext.each(data, function(r) {
                        var m = {},
                            fieldInstanceFn;
                        if (me.getFirstUnit() && r['Q_Unit'] === me.getFirstUnit()) {
                            fieldInstanceFn = me.getFirstFieldInstance;
                        } else if (me.getSecondUnit() && r['Q_Unit'] === me.getSecondUnit()) {
                            fieldInstanceFn = me.getSecondFieldInstance;
                        } else {
                            return;
                        }
                        tmp[r['Q_Indicator']] = tmp[r['Q_Indicator']] || {};
                        tmp[r['Q_Indicator']][r['Q_Year']] = tmp[r['Q_Indicator']][r['Q_Year']] || {};
                        Ext.each(me.unitSpecificFields, function(f) {
                            m[fieldInstanceFn.call(me, f)] = r[f];
                            delete r[f];
                        });
                        if (!tmp[r['Q_Indicator']][r['Q_Year']][r['Q_Month']]) {
                            tmp[r['Q_Indicator']][r['Q_Year']][r['Q_Month']] = r;
                            newData.push(r);
                        }
                        Ext.merge(tmp[r['Q_Indicator']][r['Q_Year']][r['Q_Month']], m);
                    });
                    oj[this.getRootProperty()] = newData;
                    Ext.isFunction(this._extendResponseFn) && this._extendResponseFn(oj[this.getRootProperty()]);
                    return oj;
                } catch (ex) {
        ...
        !Ext.ClassManager.isCreated('QRegPV.CountView') && Ext.define('QRegPV.CountView', {
            extend: 'Ext.view.View',
            initComponent: function() {
                this.store = this.hypertoni ? 'QRegPVCountStoreHyp' : 'QRegPVCountStoreKrs';
                this.tpl = new Ext.XTemplate(
                    '<tpl for=".">',
                    this.generateTemplateEl('{Q_Varde} med {Sickness}', '{Q_Unit}, {Date}'),
                    '</tpl>'
                );
                this.emptyText = this.generateTemplateEl('Registreringar saknas', '(avser senaste tidsperiod för vald primär vårdcentral)');
                this.callParent();
            },
            loadMask: false,
            width: '100%',
            ...
            generateTemplateEl: function(mainContent, description) {
                return Ext.String.format('<div class="qreg-count-row" style="border-right: 4px solid ' + (this.hypertoni ? '#E98300' : '#614D7D') + '"">' +
        ...
        window.scrollToTop = function(offset) {
            (Ext.isChrome ? Ext.getBody() : Ext.get(document.documentElement)).scrollTo('top', typeof offset === 'number' ? offset : 0, true);
        };
    }
}};